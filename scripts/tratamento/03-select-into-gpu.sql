...
SELECT if_exist_date(mobile_release_date)                                                 AS video_integrado,
       ...
       TRIM(split_part(boost_clock, ' ', 1))::SMALLINT                                    AS clock_boost,
       ...
       split_part(NULLIF(tdp, 'unknown'), ' ', 1)::SMALLINT                               AS tdp_watts
INTO gpu
...
SELECT quantidade_chips, fabricante, nome, name, graphics_processor
FROM gpu
         JOIN datasets.specification_gpu AS spec_gpu ON UPPER(spec_gpu.name) = nome;